...
export async function confirmTranscriptAction(formData: FormData) {
  ...
  for (const entry of entries) {
    switch (entry.kind) {
      case "TRADE_ENTRY": {
        const trade = await createTradeFromEntry(transcript.transcriptDateTime, entry, transcript.tags);
      ...
      case "TRADE_EXIT": {
        ...
        await db.update("trades", tradeId, {
          ...
          emotionalState: enumFromText(EmotionalState, entry.emotionalState, existing.emotionalState ?? EmotionalState.UNKNOWN),
...
async function createTradeFromEntry(tradeDateTime: Date, entry: TradeEntryEntry, sourceTags?: string[]) {
  ...
  return db.create("trades", {
    ...
    instrument: (entry.instrument ?? "UNKNOWN").toUpperCase(),
    direction,
    ...
    emotionalState: enumFromText(EmotionalState, entry.emotionalState, EmotionalState.UNKNOWN),
    riskPosture: enumFromText(RiskPosture, entry.riskPosture, RiskPosture.UNKNOWN),
