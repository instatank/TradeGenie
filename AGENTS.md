...
- `lib/asset-history.ts`: the asset page's two pure rules — `diffBias` (what moved in the "Current view", so an overwritten thesis is kept) and `buildAssetTimeline`/`splitTimeline` (notes + bias markers + trades + symbol-tagged quick notes as one chronological column, recent shown and older folded). Store-free.
- `components/ScreenshotField.tsx`: paste-or-drop a chart into a form. Wraps a real `<input type="file" multiple>`, so it degrades to a plain picker with JS off; `lib/screenshot-storage.ts` derives the storage folder from the owner kind.
