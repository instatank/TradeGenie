import { bucketStatsFor, expectancyBreakdown, MIN_SAMPLE, type MetricTrade } from "@/lib/metrics";
...
export type ComparisonMetric = {
  ...
  format: "count" | "percent" | "r" | "money" | "score";
  a: number | null;
  b: number | null;
 ...
 *  sizes is the most common way a comparison lies, so the page says which
 *  metrics are per-trade and which are not. */
export const PER_TRADE_METRICS = new Set(["Win rate", "Expectancy", "Average trade", "Process score"]);
