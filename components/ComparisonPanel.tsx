...
import { PER_TRADE_METRICS, type Comparison, type ComparisonMetric } from "@/lib/compare";
  ...
  return (
    <section className="panel mb-4 border-l-4 border-forge-blue">
      ...
      <div className="mt-3 space-y-1.5 text-xs text-forge-muted">
        <p>
          <span className="font-medium text-forge-ink">Win rate, expectancy, average trade and process score are
          per-trade</span> — those are the fair comparison. Net P&amp;L is a total, so the side with more trades tends to
          win it whatever the quality.
        </p>
  ...
  return (
    <tr className="border-t border-forge-line">
      <td className="px-3 py-2 font-medium">
        {metric.label}
        {PER_TRADE_METRICS.has(metric.label) ? (
          <span className="ml-1.5 text-[11px] font-normal text-forge-muted">per trade</span>
        ) : null}
      </td>
